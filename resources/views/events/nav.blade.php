                ...
                <div class="row">
                    ...
                    <div class="col-md-8">
                        <ul class="cp-top-social">
                            ...
                            @if (!Auth::guest())
                                ...
                                <li class="dropdown">
                                    ...
                                    <ul class="dropdown-menu" role="menu">
                                        <li><a href="{{ url('events/me') }}"><i class="fa fa-btn fa-user"></i> &nbsp; Basic Information</a></li>
                                        @if(!Auth::user()->info)
                                            <li><a href="{{ url('events/becomeProvider') }}"><i class="fa fa-btn fa-user"></i> &nbsp; Become a Service Provider</a></li>
                                        @else
                                            <li><a href="{{ url('events/myprofile') }}"><i class="fa fa-btn fa-address-card-o"></i> &nbsp; Advance Information</a></li>
                                            @if(count(Auth::user()->events) > 0)
                                                <li><a href="{{ url('events/myevent') }}"><i class="fa fa-btn fa-dashboard"></i> &nbsp; Dashboard</a></li>
                                            @endif
                                            @if(count(Auth::user()->serviceOrders) > 0)
                                                <li><a href="{{ url('events/myorders') }}"><i class="fa fa-btn fa-briefcase"></i> &nbsp; Manage My Service Orders</a></li>
                                            @endif
                                        @endif
                            ...
                            @else
                                <li class="dropdown">
                                    <a href="#" class="dropdown-toggle quote-btn"  data-toggle="modal" data-target="#loginModal" role="button" aria-haspopup="true" aria-expanded="false">
                                        Login
                                    </a>
        ...
        <div class="modal fade" id="loginModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
            <div class="modal-dialog  modal-sm" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                        <h4 class="modal-title" id="myModalLabel">Login to Youpple events</h4>
                    </div>
                    <div class="modal-body" style="padding: 0px 0px">
                        <form action="{{ url('events/login') }}" method="post">
                            {{ csrf_field() }}
                            <div style="padding: 10px" id="ajaxx">
                                @if (count($errors) > 0)
                                ...
                                <button class="btn btn-submit btn-lg btn-block">Login</button>
                                <hr />
