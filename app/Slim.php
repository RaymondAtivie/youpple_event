...
class Slim {
    ...
    private static function parseInput($value) {
        ...
        if (isset ($data->input)) {
            ...
            $ext = pathinfo($data->input->name, PATHINFO_EXTENSION);
            $input = array(
                'data' => $inputData,
    ...
    public static function saveFile($data,$name, $ext,$old, $path = 'tmp/', $uid = true) {
        ...
        // store the file
        $saved = Slim::save($data, $newpath,$oldpath);
        if ($saved == 'new') {
          $name = $name;
          $path = $newpath;
        }else{
     ...
     * @param $path
     */
    private static function save($data, $path, $oldpath) {
